    ...
    public static class SplatterUtils {
        public static Vector3 GetRandomDirection(SplatterSettings settings, Vector3? avgDir) {
            ...
            switch (currentDirectionMode) {
                case DirectionMode.Vector:
                    ...
                    movAngle += random;
                    moveDir = settings.orientation == OrientationMode.Vertical
                        ? new Vector3(Mathf.Cos(movAngle), Mathf.Sin(movAngle), 0f)
                        : new Vector3(Mathf.Cos(movAngle), 0f, Mathf.Sin(movAngle));
                    break;
                case DirectionMode.Transform:
                    if (settings.directionReference == null) {
                        Debug.LogError("<b>[Stain System]</b> Orientation reference is not set.");
                        return Vector3.zero;
                    }
                    ...
                    moveDir = settings.orientation == OrientationMode.Vertical
                        ? new Vector3(Mathf.Cos(angles.z), Mathf.Sin(angles.z), 0f)
                        : new Vector3(Mathf.Cos(angles.z), 0f, Mathf.Sin(angles.z));
                    break;
                ...
                switch (settings.colorMode) {
                    ...
                    case ColorMode.List:
                        if (settings.colors.Length == 0) {
                            Debug.LogError("<b>[Stain System]</b> No colors are defined in the colors list.");
                            return result;
                        }
        ...
        public static void SpawnBranch(GameObject splatterBranchPrefab, Transform branchParent, 
            ...
            for (int i = 0; i < numBranches; i++) {
                Color c = GetRandomColor(settings, color);
                Vector3 dir = GetRandomDirection(settings, direction);
